...
# Score bands. Deliberately span the WHOLE range, including the bottom: names
# the model rates PASS and AVOID are the control group, and without them the
# cohort cannot falsify anything.
...
# Buyer-depth bands. Zero is its own band because "no identifiable buyer" is a
# qualitatively different state, not just a low count.
DEPTH_BANDS: tuple[tuple[str, int, int], ...] = (
    ("buyers_0", 0, 0),
    ("buyers_1_2", 1, 2),
    ("buyers_3_9", 3, 9),
...
def depth_band(buyer_count: int) -> str:
    for name, low, high in DEPTH_BANDS:
        if low <= buyer_count <= high:
            return name
    return DEPTH_BANDS[-1][0]


def stratum_key(score: float, buyer_count: int) -> str:
    return f"{score_band(score)}|{depth_band(buyer_count)}"
...
def plan_sample(session: Session, *, size: int, cohort: str,
    ...
    for score, domain in rows:
        ...
        if score.acquisition_price is None or score.acquisition_price <= 0:
            continue
        cells[stratum_key(score.score, score.buyer_count)].append((score, domain))

    plan.eligible = sum(len(v) for v in cells.values())
    ...
    for key in sorted(cells):
        ...
        requested = allocation.get(key, 0)
        plan.cells.append(CellPlan(
            stratum=key, score_band=score_name, depth_band=depth_name,
            available=available, requested=requested, sampled=requested,
            shortfall=0))
...
def _add_balance_warnings(plan: SamplePlan) -> None:
    ...
    score_bands_present = {c.score_band for c in filled}
    depth_bands_present = {c.depth_band for c in filled}

    missing_scores = {name for name, _, _ in SCORE_BANDS} - score_bands_present
    if missing_scores:
        plan.warnings.append(
            f"No positions in score band(s): {', '.join(sorted(missing_scores))}. "
            f"The cohort does not span the score range, so recall cannot be "
            f"measured for the missing bands.")

    missing_depths = {name for name, _, _ in DEPTH_BANDS} - depth_bands_present
    if missing_depths:
        plan.warnings.append(
            f"No positions in buyer-depth band(s): "
            f"{', '.join(sorted(missing_depths))}. Buyer depth will be "
            f"partially confounded with score in this cohort.")
...
def cohort_health(session: Session, cohort: str | None = None) -> CohortHealth:
    ...
    for position in positions:
        ...
        s_band = score_band(score) if score is not None else "unknown"
        d_band = depth_band(int(depth)) if depth is not None else "unknown"
