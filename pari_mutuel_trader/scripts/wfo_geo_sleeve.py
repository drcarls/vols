...
def attach_dated_exit(feat: pd.DataFrame, half_life_weeks: float = 13.0) -> pd.DataFrame:
    """Dated events WITH an exit rule (decay). Thin wrapper over the library's canonical
    ``attach_geo_signal_timeline`` so the research numbers and the shipped feature stay identical.
    ONE fixed half-life for ALL events (not per-event tuned) to avoid fitting the windows.
    """
    from pari_mutuel_trader.data.geopolitical import attach_geo_signal_timeline
    tl = [{"event": e, "start": s, "end": en, "prob": p, "premium": q}
          for (s, en, e, p, q) in TIMELINE]
    return attach_geo_signal_timeline(feat, tl, half_life_weeks=half_life_weeks)
